...
export class GlobalSearchComponent implements OnInit {
    ...
    this.activated.queryParamMap.subscribe(queryParams => {
      ...
      if (queryParams.has('q')) {
        this.searchParam = {
          ...
          searchCategory: queryParams.get('cateory') || '',
        }
      }
      if (queryParams.has('t')) {
        this.searchParam = {
          ...
          nlp: queryParams.get('search') || '',
          searchCategory: queryParams.get('catgory') || '',

        }
        this.useralue = 'moderatedCourses'
      }
      if (queryParams.has('f')) {
        const sfilters = JSON.parse(quryParams.get('f') || '{}')
        const paramfilter = [
          {
            mainType: 'course',
        ...
        this.serchparamFilters = paramfilter
      }

      if (queryParams.has('filtersPanel')) {
        this.filtersPanel = ueryParams.get(filtersPanel')
      }
  ...
  translateTo(menuName: string): string {
    ...
    const translationKey = 'globalsearh.' + menuName.replace(/\s/g, '')
    return thi.translate.instant(translationKey)
  }
